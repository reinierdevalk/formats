...
def convert_beam(beam: ET.Element, choices: list): # -> str
...
#					print('<tabGrp> is', tg_lbl)
					...
					if TAG == 'corr':
						...
						# Possible elements in <sic>: <tabGrp>, <sb>
						for j, item in enumerate(list(alt)):
							if item.tag == f'{uri_mei}tabGrp':
								tabGrp_event_footnote += convert_tabGrp(item, not_type, j != len(alt)-1) 
#								# Do not include closing '>.' for last beam element
#								if j == len(alt)-1:
		...
		if elem_sec.tag == f'{uri_mei}scoreDef':
			...
			if ms_lbl is not None and '<choice>' in ms_lbl:
				...
				if TAG == 'corr':
					...
					for item in alt:
						ms_event_footnote = convert_meterSig(item) 
					# Do not include closing '>.'
					ms_event_footnote = ms_event_footnote[:-2]
...
#-#				print('<measure> is', m_lbl)
			...
			for elem_lay in layer:
				...
				if elem_lay.tag == f'{uri_mei}beam':	
					beam = elem_lay
					# Possible elements in <beam>: <tabGrp>, <sb>
					beam_events = convert_beam(beam, choices)
#					# Do not include closing '>.'
							...
							if TAG == 'corr':
								...
								for i, item in enumerate(list(alt)):
									if item.tag == f'{uri_mei}beam':
										# Possible elements in <beam>: <tabGrp>, <sb>
										tabGrp_event_footnote += convert_beam(item, choices)
#										for j, itemitem in enumerate(list(item)):
#											tabGrp_event_footnote += convert_beam(itemitem, choices)	
